...
std::unique_ptr<RamRelationReference> AstTranslator::translateDeltaDiffMinusRelation(
        const AstRelation* rel) {
    return translateRelation(rel, "@delta_diff_minus@_");
}

std::unique_ptr<RamRelationReference> AstTranslator::translateDeltaDiffPlusRelation(
        const AstRelation* rel) {
    return translateRelation(rel, "@delta_diff_plus@_");
}
            ...
            if (isReinsertionRule) {
                ...
                if (isInsertionRule) {
                    for (size_t i = 0; i < atoms.size(); i++) {
                        ...
                        cl->getAtoms()[i]->setName(translateDiffPlusRelation(getAtomRelation(atoms[i], program))->get()->getName());
                    ...
                    */
                    ...
                    for (size_t i = 0; i < atoms.size(); i++) {
                        ...
                        cl->getAtoms()[i]->setName(translateDiffMinusRelation(getAtomRelation(atoms[i], program))->get()->getName());
    ...
    for (const AstRelation* rel : scc) {
            ...
            appendStmt(updateRelTable,
                    std::make_unique<RamSequence>(
                            ...
                            std::make_unique<RamMerge>(
                                    ...
                                    std::unique_ptr<RamRelationReference>(translateNewDiffPlusRelation(rel)->clone())),
                                    */
                            // merge from delta_relation into diff_plus, but only tuples already existing
                            ...
                            std::make_unique<RamSemiMerge>(
                                    ...
                                    std::unique_ptr<RamRelationReference>(translateDeltaRelation(rel)->clone()),
                                    std::unique_ptr<RamRelationReference>(translateNewDiffPlusRelation(rel)->clone())),
                            // merge from new_diff_minus into diff_plus, but only tuples already existing
                            // again, only updating counts
                            std::make_unique<RamSemiMerge>(
                                    std::unique_ptr<RamRelationReference>(translateDiffPlusRelation(rel)->clone()),
                                    std::unique_ptr<RamRelationReference>(translateNewDiffMinusRelation(rel)->clone()),
                                    std::unique_ptr<RamRelationReference>(translateNewDiffPlusRelation(rel)->clone()), true),
                            ...
                            std::make_unique<RamSemiMerge>(
                                    std::unique_ptr<RamRelationReference>(translateDiffMinusRelation(rel)->clone()),
                                    std::unique_ptr<RamRelationReference>(translateNewDiffPlusRelation(rel)->clone()),
                                    std::unique_ptr<RamRelationReference>(translateNewDiffMinusRelation(rel)->clone()), true),
                            ...
                            // create the delta diff plus relations with correct counts
                            // don't do a positive-merge directly from diff_plus for speed reasons
                            // merge new tuples from new_diff_plus
                            std::make_unique<RamMerge>(
                                    std::unique_ptr<RamRelationReference>(translateDeltaDiffPlusRelation(rel)->clone()),
                                    std::unique_ptr<RamRelationReference>(translateNewDiffPlusRelation(rel)->clone())),
                            // update their counts from diff_minus_applied
                            std::make_unique<RamSemiMerge>(
                                    std::unique_ptr<RamRelationReference>(translateDeltaDiffPlusRelation(rel)->clone()),
                                    std::unique_ptr<RamRelationReference>(translateDiffMinusAppliedRelation(rel)->clone())),

                            // create the delta diff minus relations
                            // merge new tuples from new_diff_minus
                            std::make_unique<RamMerge>(
                                    std::unique_ptr<RamRelationReference>(translateDeltaDiffMinusRelation(rel)->clone()),
                                    std::unique_ptr<RamRelationReference>(translateNewDiffMinusRelation(rel)->clone())),
                            // update their counts from delta and new_diff_plus
                            std::make_unique<RamSemiMerge>(
                                    std::unique_ptr<RamRelationReference>(translateDeltaDiffMinusRelation(rel)->clone()),
                                    std::unique_ptr<RamRelationReference>(translateDeltaRelation(rel)->clone())),
                            std::make_unique<RamSemiMerge>(
        ...
        */
        ...
        if (Global::config().has("incremental")) {
            ...
            // update counts in the diff minus relations, this has to go before updating diff_plus because we use the pre-updated values here
            appendStmt(preamble,
                    std::make_unique<RamSemiMerge>(std::unique_ptr<RamRelationReference>(translateDiffMinusRelation(rel)->clone()),
                            std::unique_ptr<RamRelationReference>(translateDeltaRelation(rel)->clone())));
            appendStmt(preamble,
                    std::make_unique<RamSemiMerge>(std::unique_ptr<RamRelationReference>(translateDiffMinusRelation(rel)->clone()),
                            std::unique_ptr<RamRelationReference>(translateDiffPlusRelation(rel)->clone())));
            ...
            // update counts in the diff plus relations
            appendStmt(preamble,
                    std::make_unique<RamSemiMerge>(std::unique_ptr<RamRelationReference>(translateDiffPlusRelation(rel)->clone()),
                            std::unique_ptr<RamRelationReference>(translateDiffMinusAppliedRelation(rel)->clone())));
            appendStmt(preamble,
                    std::make_unique<RamMerge>(
                            std::unique_ptr<RamRelationReference>(translateDeltaDiffPlusRelation(rel)->clone()),
            ...
            /*
            // populate the diff count relations
            appendStmt(preamble,
                    ...
                    // for (size_t j = 0; j < atoms.size(); j++) {
                        ...
                        if (isInsertionRule) {
                            ...
                            for (size_t i = 0; i < atoms.size(); i++) {
                                ...
                                rdiff->getAtoms()[i]->setName(translateDiffPlusRelation(getAtomRelation(atoms[i], program))->get()->getName());
                            ...
                            for (size_t i = 0; i < atoms.size(); i++) {
                                ...
                                for (size_t j = 0; j < atoms.size(); j++) {
                                    ...
                                    if (j < i) {
                                        ...
                                        r1->getAtoms()[j]->setName(translateDeltaDiffMinusRelation(getAtomRelation(atoms[j], program))->get()->getName());
                                    } else if (j > i) {
...
std::unique_ptr<RamStatement> AstTranslator::makeIncrementalCleanupSubroutine(const AstProgram& program) {
    ...
    for (const auto& relation : program.getRelations()) {
        ...
        // the subroutine needs to be built from inside out
        // build the insertion step first
        std::vector<std::unique_ptr<RamExpression>> updateTupleDiffPlus;
        
        // insert the original tuple
        for (size_t i = 0; i < relation->getArity() - 2; i++) {
            updateTupleDiffPlus.push_back(std::make_unique<RamTupleElement>(0, i));
        }

        std::vector<std::unique_ptr<RamExpression>> counts = {std::make_unique<RamTupleElement>(0, relation->getArity() - 1), std::make_unique<RamTupleElement>(0, relation->getArity() - 2)};
        std::vector<std::unique_ptr<RamExpression>> counts2 = {std::make_unique<RamTupleElement>(0, relation->getArity() - 1), std::make_unique<RamTupleElement>(0, relation->getArity() - 2)};
        // auto prevCount = std::make_unique<RamTupleElement>(0, relation->getArity() - 2);
        // auto curCount = std::make_unique<RamTupleElement>(0, relation->getArity() - 1);

        // insert cur_count - prev_count for both counts
        // updateTupleDiffPlus.push_back(std::unique_ptr<RamExpression>(new RamIntrinsicOperator(FunctorOp::SUB, std::unique_ptr<RamTupleElement>(curCount->clone()), std::unique_ptr<RamTupleElement>(prevCount->clone()))));
        // updateTupleDiffPlus.push_back(std::unique_ptr<RamExpression>(new RamIntrinsicOperator(FunctorOp::SUB, std::unique_ptr<RamTupleElement>(curCount->clone()), std::unique_ptr<RamTupleElement>(prevCount->clone()))));

        // updateTupleDiffPlus.push_back(std::make_unique<RamIntrinsicOperator>(FunctorOp::SUB, std::unique_ptr<RamTupleElement>(curCount->clone()), std::unique_ptr<RamTupleElement>(prevCount->clone())));
        // updateTupleDiffPlus.push_back(std::make_unique<RamIntrinsicOperator>(FunctorOp::SUB, counts));
        updateTupleDiffPlus.push_back(std::make_unique<RamNumber>(0));
        updateTupleDiffPlus.push_back(std::make_unique<RamIntrinsicOperator>(FunctorOp::SUB, counts2));

        // create the projection
        auto insertUpdateDiffPlus = std::make_unique<RamProject>(std::unique_ptr<RamRelationReference>(translateDiffMinusAppliedRelation(relation)->clone()), std::move(updateTupleDiffPlus));

        // create the scan
        auto cleanupScanDiffPlus = std::make_unique<RamScan>(std::unique_ptr<RamRelationReference>(translateDiffPlusRelation(relation)->clone()), 0, std::move(insertUpdateDiffPlus));
        appendStmt(cleanupSequence, std::make_unique<RamQuery>(std::move(cleanupScanDiffPlus)));
...
void AstTranslator::translateProgram(const AstTranslationUnit& translationUnit) {
    ...
    for (const auto& scc : sccOrder.order()) {
        ...
        if (Global::config().has("incremental")) {
            if (isRecursive) {
                // merge any input relations correctly
                for (const auto& relation : internIns) {
