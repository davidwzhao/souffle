...
int RamLevelAnalysis::getLevel(const RamNode* node) const {
    ...
    public:
        ...
        // auto increment
        int visitIterationNumber(const RamIterationNumber& increment) override {
            return -1;
        }
        ...
        // existence check
        int visitPositiveExistenceCheck(const RamPositiveExistenceCheck& exists) override {
            int level = -1;
            for (const auto& cur : exists.getValues()) {
                level = std::max(level, visit(cur));
            }
        ...
        int visitSubsumptionExistenceCheck(const RamSubsumptionExistenceCheck& subsumptionExists) override {
            int level = -1;
            for (const auto& cur : subsumptionExists.getValues()) {
                level = std::max(level, visit(cur));
            }
            return level;
        }
