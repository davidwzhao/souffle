...
std::vector<AstClause*> IncrementalTransformer::makeNegativeUpdateClause(const AstClause& clause, const AstTranslationUnit& translationUnit) {
    ...
    for (size_t i = 0; i < negativeUpdateClause->getBodyLiterals().size(); i++) {
        ...
        if (auto atom = dynamic_cast<AstAtom*>(lit)) {
            ...
            auto headRelation = getRelation(*translationUnit.getProgram(), clause.getHead()->getQualifiedName());
            if (contains(sccGraph.getInternalRelations(sccGraph.getSCC(headRelation)), getRelation(*translationUnit.getProgram(), atom->getQualifiedName()))) {
    ...
    // negativeUpdateClause->getHead()->addArgument(std::make_unique<AstIntrinsicFunctor>(FunctorOp::ADD, std::make_unique<AstNumericConstant>(1), std::unique_ptr<AstArgument>(applyFunctorToVars(bodyLevels, FunctorOp::MAX))));
    ...
    if (recursiveClauses.recursive(&clause)) {
        ...
        negativeUpdateClause->getHead()->addArgument(std::make_unique<AstNumericConstant>("0"));
    }

    // second is the previous epoch's count, which we set to 0 signifying that we are updating the head tuple
    negativeUpdateClause->getHead()->addArgument(std::make_unique<AstNumericConstant>("1"));
    ...
    if (bodyLevels.size() > 0) {
        ...
        negativeUpdateClause->addToBody(std::make_unique<AstBinaryConstraint>(BinaryConstraintOp::EQ,
                    ...
                    std::make_unique<AstIntrinsicFunctor>(FunctorOp::SUB, std::make_unique<AstIterationNumber>(), std::make_unique<AstNumericConstant>("1"))));
    }
...
std::vector<AstClause*> IncrementalTransformer::makePositiveUpdateClause(const AstClause& clause, const AstTranslationUnit& translationUnit) {
    ...
    for (size_t i = 0; i < positiveUpdateClause->getBodyLiterals().size(); i++) {
        ...
        if (auto atom = dynamic_cast<AstAtom*>(lit)) {
            ...
            bodyCountDiffs.push_back((new AstIntrinsicFunctor(FunctorOp::LAND, 
                            (std::make_unique<AstIntrinsicFunctor>(FunctorOp::LNOT, std::make_unique<AstIntrinsicFunctor>(FunctorOp::BOR, std::make_unique<AstVariable>("@prev_count_" + std::to_string(i)), std::make_unique<AstNumericConstant>(0)))),
    ...
    // add three incremental columns to head lit
    // first is the iteration number, which we get by adding 1 to the max iteration number over the body atoms
    // positiveUpdateClause->getHead()->addArgument(std::make_unique<AstIntrinsicFunctor>(FunctorOp::ADD, std::make_unique<AstNumericConstant>(1), std::unique_ptr<AstArgument>(applyFunctorToVars(bodyLevels, FunctorOp::MAX))));
    ...
    if (recursiveClauses.recursive(&clause)) {
        ...
        positiveUpdateClause->getHead()->addArgument(std::make_unique<AstNumericConstant>("0"));
    }
    ...
    positiveUpdateClause->getHead()->addArgument(std::make_unique<AstNumericConstant>("1"));
...
std::unique_ptr<AstClause> IncrementalTransformer::makePositiveGenerationClause(const AstClause& clause, const AstTranslationUnit& translationUnit) {
    ...
    // add three incremental columns to head lit
    // first is the iteration number, which we get by adding 1 to the max iteration number over the body atoms
    // positiveGenerationClause->getHead()->addArgument(std::make_unique<AstIntrinsicFunctor>(FunctorOp::ADD, std::make_unique<AstNumericConstant>(1), std::unique_ptr<AstArgument>(applyFunctorToVars(bodyLevels, FunctorOp::MAX))));
    ...
    if (recursiveClauses.recursive(&clause)) {
        ...
        positiveGenerationClause->getHead()->addArgument(std::make_unique<AstNumericConstant>("0"));
    }
    ...
    positiveGenerationClause->getHead()->addArgument(std::make_unique<AstNumericConstant>("1"));
...
bool IncrementalTransformer::transform(AstTranslationUnit& translationUnit) {
    ...
    for (auto relation : program->getRelations()) {
        relation->addAttribute(
                std::make_unique<AstAttribute>(std::string("@iteration"), AstQualifiedName("number")));
        relation->addAttribute(
                std::make_unique<AstAttribute>(std::string("@prev_count"), AstQualifiedName("number")));
        relation->addAttribute(
                std::make_unique<AstAttribute>(std::string("@current_count"), AstQualifiedName("number")));
    ...
    for (auto relation : originalRelations) {
        ...
        for (auto clause : getClauses(*program, relation->getQualifiedName())) {
            ...
            if (isFact(*clause)) {
                ...
                program->addClause(std::unique_ptr<AstClause>(makePositiveGenerationClause(*clause, translationUnit)));
