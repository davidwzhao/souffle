...
/**
 * @class RamPositiveExistenceCheck
 ...
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~
 */
class RamPositiveExistenceCheck : public RamAbstractExistenceCheck {
public:
    RamPositiveExistenceCheck(
            std::unique_ptr<RamRelationReference> relRef, std::vector<std::unique_ptr<RamExpression>> vals)
            : RamAbstractExistenceCheck(std::move(relRef), std::move(vals)) {}
    ...
    void print(std::ostream& os) const override {
        os << "("
           ...
           << ") pos∈ " << getRelation().getName();
    }

    RamPositiveExistenceCheck* clone() const override {
        std::vector<std::unique_ptr<RamExpression>> newValues;
        for (auto& cur : values) {
            newValues.emplace_back(cur->clone());
        }
        return new RamPositiveExistenceCheck(
                std::unique_ptr<RamRelationReference>(relationRef->clone()), std::move(newValues));
    }
};
